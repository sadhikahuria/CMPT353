...
def pivot_months_pandas(data):
    ...
    precip = grouped_data.aggregate({'precipitation': 'sum'}).reset_index()
    monthly = precip.pivot(index='name', columns='month', values='precipitation')

    obs = grouped_data.aggregate({'precipitation': 'count'}).reset_index()
    counts = obs.pivot(index='name', columns='month', values='precipitation')
